import { t as getDigFormat } from "./main.min.js";
/* empty css             */
...
//#endregion
//#region node_modules/lightgallery/lightgallery.es5.js
/*!
...
var lgQuery = function() {
	...
	lgQuery.prototype._getSelector = function(selector, context) {
		...
		if (selector.substring(0, 1) === "#") return context.querySelector(selector);
	...
	lgQuery.prototype._getFirstEl = function() {
		if (this.selector && this.selector.length !== void 0) return this.selector[0];
	...
	lgQuery.prototype.first = function() {
		if (this.selector && this.selector.length !== void 0) return $LG(this.selector[0]);
	...
	lgQuery.prototype.trigger = function(event, detail) {
		if (!this.firstElement) return this;
		var customEvent = new CustomEvent(event.split(".")[0], { detail: detail || null });
	...
	lgQuery.prototype.prepend = function(html) {
		this._each(function(el) {
			...
			else if (html instanceof HTMLElement) el.insertBefore(html.cloneNode(true), el.firstChild);
		});
		return this;
	};
...
var utils = {
	...
	*/
	getSize: function(el, container, spacing, defaultLgSize) {
		...
		var lgSize = $LG(el).attr("data-lg-size") || defaultLgSize;
		if (!lgSize) return;
		...
		if (isResponsiveSizes[1]) {
			...
			for (var i = 0; i < isResponsiveSizes.length; i++) {
				...
				if (parseInt(size_1.split("-")[2], 10) > wWidth) {
					lgSize = size_1;
					break;
				}
	...
	*/
	getTransform: function(el, container, top, bottom, imageSize) {
		...
		return "translate3d(" + (x *= -1) + "px, " + (y *= -1) + "px, 0) scale3d(" + scX + ", " + scY + ", 1)";
	},
	...
	getImgMarkup: function(index, src, altAttr, srcset, sizes, sources) {
		...
		var sourceTag = "";
		if (sources) sourceTag = (typeof sources === "string" ? JSON.parse(sources) : sources).map(function(source) {
			var attrs = "";
	...
	*/
	getDynamicOptions: function(items, extraProps, getCaptionFromTitleOrAlt, exThumbImage) {
		...
		[].forEach.call(items, function(item) {
			...
			dynamicEl.thumb = exThumbImage ? currentItem.attr(exThumbImage) : currentItem.find("img").first().attr("src");
	...
	*/
	isVideo: function(src, isHTML5VIdeo, index) {
		if (!src) {
			if (isHTML5VIdeo) return { html5: true };
			else {
		...
		else if (vimeo) return { vimeo };
		else if (wistia) return { wistia };
	}
};
var lgId = 0;
var LightGallery = function() {
	...
	LightGallery.prototype.normalizeSettings = function() {
		...
		if (this.settings.container) {
			...
			if (typeof container === "function") this.settings.container = container();
	...
	LightGallery.prototype.init = function() {
		...
		this.buildStructure();
		this.LGel.trigger(lGEvents.init, { instance: this });
	...
	*/
	...
	LightGallery.prototype.buildStructure = function() {
		var _this = this;
		if (this.$container && this.$container.get()) return;
		var controls = "";
		var subHtmlCont = "";
	...
	LightGallery.prototype.refreshOnResize = function() {
		if (this.lgOpened) {
			var __slideVideoInfo = this.galleryItems[this.index].__slideVideoInfo;
	...
	LightGallery.prototype.resizeVideoSlide = function(index, imageSize) {
		...
		this.getSlideItem(index).find(".lg-video-cont").attr("style", lgVideoStyle);
	};
	/**
	...
	*/
	LightGallery.prototype.getMediaContainerPosition = function() {
		...
		return {
			top,
			bottom: (thumbContainer ? thumbContainer.clientHeight : 0) + captionHeight
		};
	};
	...
	*/
	LightGallery.prototype.addHtml = function(index) {
		...
		if (this.settings.appendSubHtmlTo !== ".lg-item") {
			if (subHtmlUrl) utils.fetchCaptionFromUrl(subHtmlUrl, this.outer.find(".lg-sub-html"), "replace");
			else this.outer.find(".lg-sub-html").html(subHtml);
		} else {
			var currentSlide = $LG(this.getSlideItemId(index));
			if (subHtmlUrl) utils.fetchCaptionFromUrl(subHtmlUrl, currentSlide, "append");
		...
		this.LGel.trigger(lGEvents.afterAppendSubHtml, { index });
	};
	/**
	*  @desc Preload slides
	*  @param {Number} index - index of the slide
	...
	LightGallery.prototype.loadContent = function(index, rec) {
		...
		if (!$currentSlide.hasClass("lg-loaded")) {
			...
			if (iframe) {
				...
				var dummyImg = "";
				if (isFirstSlide && this.zoomFromOrigin && this.currentImageSize) dummyImg = this.getDummyImageContent($currentSlide, index, "");
	...
	*/
	...
	LightGallery.prototype.setDownloadValue = function(index) {
		if (this.settings.download) {
			var currentGalleryItem = this.galleryItems[index];
			if (currentGalleryItem.downloadUrl === false || currentGalleryItem.downloadUrl === "false") this.outer.addClass("lg-hide-download");
			else {
	...
	*/
	...
	LightGallery.prototype.touchMove = function(startCoords, endCoords, e) {
		...
		if (this.swipeDirection === "horizontal") {
			...
			var gutter = width * 15 / 100 - Math.abs(distanceX * 10 / 100);
	...
	LightGallery.prototype.goToNextSlide = function(fromTouch) {
		...
		if (!this.lgBusy) {
			if (this.index + 1 < this.galleryItems.length) {
				this.index++;
				this.LGel.trigger(lGEvents.beforeNextSlide, { index: this.index });
	...
	*/
	LightGallery.prototype.closeGallery = function(force) {
		...
		setTimeout(function() {
			...
			_this.$inner.empty();
			if (_this.lgOpened) _this.LGel.trigger(lGEvents.afterClose, { instance: _this });
...
//#endregion
//#region src/components/layout/gallery/gallery.js
var KEY = "7EC452A9-0CFD441C-BD984C7C-17C8456E";
function initGallery() {
	if (document.querySelector("[data-fls-gallery]")) new lightGallery(document.querySelector("[data-fls-gallery]"), {
		licenseKey: KEY,
		selector: "a",
		speed: 500
	});
...
//#endregion
//#region src/components/custom/portfolio/portfolio.js
var portfolio = document.querySelector(".portfolio");
var galleryPortfolio = document.querySelector(".gallery-portfolio");
portfolio.addEventListener("click", portfolioActions);
function portfolioActions(e) {
	const targetElement = e.target;
	if (targetElement.closest("[data-portfolio-filter]")) {
		const currentElement = targetElement.closest("[data-portfolio-filter]");
		if (!currentElement.classList.contains("filter-portfolio__button--active")) {
			galleryPortfolio.classList.add("gallery-portfolio--animate");
			setTimeout(() => {
				galleryPortfolio.classList.remove("gallery-portfolio--animate");
			}, 600);
			document.querySelector(".filter-portfolio__button--active").classList.remove("filter-portfolio__button--active");
			currentElement.classList.add("filter-portfolio__button--active");
			setTimeout(() => {
				const currentElementValue = currentElement.dataset.portfolioFilter;
				const portfolioStyle = document.querySelector(".portfolio>style");
				portfolioStyle && portfolioStyle.remove();
				if (currentElementValue !== "all") {
					let style = `
						<style>
							[data-portfolio-filter-item]:not([data-portfolio-filter-item="${currentElementValue}"]) {
								display: none;
							}
						</style>`;
					portfolio.insertAdjacentHTML("afterbegin", style);
				}
			}, 400);
		}
	}
