...
function addToCart() {
	...
	if (!cart || !cartList || !cartCount) return;
	...
	function handleCartActions(e) {
		...
		if (target.closest("[data-fls-addtocart-button]")) {
			...
			const price = parsePrice(product.querySelector("[data-fls-addtocart-price]").textContent);
			...
			const flySpeed = parseInt(flyImg.dataset.flsAddtocartImage, 10) || 500;
	...
	function addToCartImageFly(imageEl, cartIcon, duration) {
		...
		flyImg.style.cssText = `
			position: absolute;
			left: ${imageEl.getBoundingClientRect().left + window.scrollX}px;
			top: ${imageEl.getBoundingClientRect().top + window.scrollY}px;
		...
		requestAnimationFrame(() => {
			...
			flyImg.style.transition = `all ${duration}ms cubic-bezier(0.42, 0, 0.58, 1)`;
	...
	function addToCartItem(title, image, price, quantity) {
		...
		if (existingItem) {
			...
			priceEl.textContent = formatPrice(price * newQuantity);
		} else {
			const li = document.createElement("li");
			li.classList.add("cart__item", "item-cart");
			li.setAttribute("data-base-price", price);
			li.innerHTML = `
				<figure class="item-cart__picture">
					<img class="item-cart__image" src="${image}" alt="${title}">
				</figure>
				<div class="item-cart__body">
					<h4 class="item-cart__title">${title}</h4>
					...
					<div data-fls-addtocart-price class="item-cart__price">${formatPrice(price * quantity)}</div>
	...
	function changeItemQuantity(item, delta) {
		...
		priceEl.textContent = formatPrice(parseFloat(item.getAttribute("data-base-price")) * quantity);
		updateCartState();
	}
	function updateCartState() {
		...
		cartItems.forEach((item) => {
			const quantity = parseInt(item.querySelector("[data-fls-quantity-value]").value, 10);
			const price = parsePrice(item.querySelector("[data-fls-addtocart-price]").textContent);
			totalCount += quantity;
			totalPrice += price;
		});
		...
		if (cartBody) {
			if (cartItems.length === 0) cartBody.classList.add("cart__body--empty");
...
//#endregion
export { isMobile as a, slideToggle as c, getHash as i, slideUp as l, addTouchAttr as n, setHash as o, dataMediaQueries as r, slideDown as s, formValidate as t };
